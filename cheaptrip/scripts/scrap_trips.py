...
#from send_event_kinesis import send_event #used to load data into AWS
...
filter_total_transits_per_option = 999 # filter total transits per trip option
filter_transits_types = ['flight']
...
"""
 below remarked variables used to load data 
 into DynamoDB using Kinesis DataStream
"""	

# table = 'tripsGenerator'
# dynamodb = boto3.resource('dynamodb')
# table = dynamodb.Table(table)
cur_date = dt.datetime.utcnow().isoformat()
# stream = 'stream_trips_generator_kinesis_dynamodb'
# pk = 'trip'    
# sk = 'entity_id'
    ...
    for option_id, option in enumerate(trip_list):
        ...
        for trans_id, transit in enumerate(option[8]):
            ...
            for trans_info_id, trans_info in enumerate(option[8][trans_id]):
                ...
                if option[8][trans_id][trans_info_id] in filter_transits_types and option[8][trans_id][trans_info_id] in filter_transits_types in ["transit", "car"]:
                ...
                elif option[8][trans_id][trans_info_id] in filter_transits_types and option[8][trans_id][trans_info_id] == "walk":
                    list_transits_found.append(option[8][trans_id][trans_info_id])
                ...
                elif option[8][trans_id][trans_info_id] in filter_transits_types and option[8][trans_id][trans_info_id] == "flight":
                    for fl_id, fl_info in enumerate(option[8][trans_id]):
                        if fl_id in (2, 3):
                            ...
                            # append_dicts(dict_flights, "" + str(trans_id) + "", dict_trans_value)

                elif option[8][trans_id][trans_info_id] in filter_transits_types and option[8][trans_id][trans_info_id] == "hotel":
                    list_transits_found.append(option[8][trans_id][trans_info_id])
                ...
                elif option[8][trans_id][trans_info_id] in filter_transits_types and option[8][trans_id][trans_info_id] == "car":
                    list_transits_found.append(option[8][trans_id][trans_info_id])
        ...
        # print(option_id, type(dict_transits), dict_transits)
        dict_option["" + trip+'#'+str(option_id) + ""]['transits'] = dict_transits
            # dict_option["" + trip+'#'+str(option_id) + ""]['flights'] = dict_flights
        # logging.info(f'trip: {trip} - option: {str(option_id+1)} contains: {len(dict_transits)} transits')
            ...
            if full_path == map_path:
                ...
                for trip_option_id, option_transits in trip_options_transits_dict.items():
                    ...
                    if trip_option_id in ls_min_price_options and len(trip_options_transits_dict[trip_option_id]["transits"].keys()) <= filter_total_transits_per_option:
                        ...
                        output_json_file_name_path = base_json_output_path + '\\'+origin+'#'+dest+'#OPTION'+trip_option_id[str.find(trip_option_id, '#')+1:]+'.json'
...
def main():
    ...
    log_file_path = str(log_dir_path) + r'/trips_python_log_' \
